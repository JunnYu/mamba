...
causal_conv1d_fwd(const paddle::Tensor &x, const paddle::Tensor &weight,
    ...
    DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(x.type(), "causal_conv1d_fwd", [&] {
        DISPATCH_WTYPE_FLOAT_AND_HALF_AND_BF16(weight.type(), "causal_conv1d_fwd", [&] {
            if (!is_channel_last) {
...
causal_conv1d_bwd(const paddle::Tensor &x, const paddle::Tensor &weight,
    ...
    // if (!is_channel_last && dout.strides()[2] != 1) { dout = dout.contiguous(); }
    // if (is_channel_last && dout.strides()[1] != 1) { dout = dout.transpose({0, 2, 1}).contiguous().transpose({0, 2, 1}); }
    ...
    DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(x.type(), "causal_conv1d_bwd", [&] {
        DISPATCH_WTYPE_FLOAT_AND_HALF_AND_BF16(weight.type(), "causal_conv1d_bwd", [&] {
            if (!is_channel_last) {
...
causal_conv1d_update(const paddle::Tensor &x,
    ...
    // Cast to char to avoid compiler warning about narrowing
    auto stream = out.stream();
    DISPATCH_ITYPE_FLOAT_AND_HALF_AND_BF16(x.type(), "causal_conv1d_update", [&] {
        DISPATCH_WTYPE_FLOAT_AND_HALF_AND_BF16(weight.type(), "causal_conv1d_update", [&] {
