...
from ..utils import custom_bwd, custom_fwd, ensure_contiguous
...
def kldiv_forward_triton(y_pred, y_true, log_target, reduction):  # [B, S]  # [B, S]
    ...
    output_tensor = paddle.zeros([out_size,], dtype=paddle.float32)
...
class KLDivLossFunction(paddle.autograd.PyLayer):
    """
    ...
    def forward(
        ...
        Args:
            ...
            y_pred (paddle.Tensor): A tensor of shape (BT, V) containing the predicted values, expected to be log-probabilities.
            y_true (paddle.Tensor): A tensor of shape (BT, V) containing the target values, expected to be either probabilities or log-probabilities, depending on the value of `log_target`.
        ...
        Returns:
            paddle.Tensor: The computed KL Divergence Loss, with shape (BT, V) if `reduction` is "none", else a scalar.
        """
