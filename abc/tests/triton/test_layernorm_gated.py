...
def test_layer_norm_gated(d, dtype, wtype, has_bias, has_z, is_rms_norm, has_group, norm_before_gate):
    ...
    if not is_rms_norm:
        if not has_group:
            ...
            out_pd = rearrange(F.layer_norm(rearrange(x_pt, "... (g d) -> ... g d", d=group_size), (group_size,), epsilon=1e-5), "... g d -> ... (g d)") * weight_pt
            if has_bias:
        ...
        out_pd = rms_norm_ref(x_pt, weight_pt, bias_pd, z=z_pt, eps=1e-5, group_size=group_size,
    ...
    if has_bias:
        assert (bias.grad - bias_ref.grad).abs().max().item() <= 2 * (bias_pd.grad - bias_ref.grad).abs().max().item() + atol
