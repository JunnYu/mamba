...
from mamba_ssm_paddle.ops.causal_conv1d_interface import causal_conv1d_fn, causal_conv1d_ref, causal_conv1d_update, causal_conv1d_update_ref
...
def test_causal_conv1d(dim, seqlen, width, has_bias, silu_activation, itype, channel_last, has_initial_states, return_final_states):
    print(dim, seqlen, width, has_bias, silu_activation, itype, channel_last, has_initial_states, return_final_states)
...
def test_causal_conv1d_update(dim, width, has_bias, silu_activation, itype):
    ...
    # batch = 1
    # dim = 64
    x = paddle.randn([batch, dim], dtype=itype)
    conv_state = paddle.randn([batch, dim, width], dtype=itype)
    weight = paddle.randn([dim, width], dtype=paddle.float32).requires_grad_()
    if has_bias:
        bias = paddle.randn([dim,], dtype=paddle.float32).requires_grad_()
    else:
